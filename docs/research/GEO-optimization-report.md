...
## 2026-09-16 更新：答案胶囊全站上线 + 对比页编辑内容补全 + GitLab 移除

本轮把 08-28 诊断以来排期的 SG 系列一次性落地，并移除了公开产品中的 GitLab 支持：

1. **全站答案胶囊（answer capsules）已上线**：在首页、文档页、对比页、报告页等核心页面顶部前置"OctoCounts 是…"式的答案胶囊段落，直接补齐原报告"可引用性与答案前置"维度（65/100）和"/compare/* 页面完全没有问答式内容（AEO 最大缺口）"两项失分点。
2. **对比页编辑内容补全至 101/101**：`/compare/*` 精选对比页的编辑内容（定义句、解读段落、可引用问答）覆盖率达到 101/101，原 P1 表格中"对比页缺问答式内容"的缺口关闭。
3. **SG-04 可复现字段已上线**：报告页新增可复现引用（仓库、代码行数、提交等字段），配合"复制引用"按钮与 `report_citation_copied` 事件，构成完整的引用 → 分发 → 回流度量链路。
4. **`/research` pilot study 上线**：第 7 节 P2 矩阵"发布原创研究"路线的首个成果已发布——/research 页面承载基于真实分析数据的原创研究内容，为站外引用与 AI 答案收录提供原创证据源。
5. **GitLab 支持从公开产品移除（2026-09-08）**：公开 GitLab 仓库分析自 2026-09-08 起从公开产品中移除，相关范围说明与路线图状态已同步更新（见 `.octocounts-roadmap-progress.json` phase-6）。
...
（以下分数基于 2026-08-28 快照；08-28 之后的整改情况见文首各日期更新节。）

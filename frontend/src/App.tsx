// The route tree: homepage (form + example report), report routes, and the
// lazy tool/marketing pages. Split from main.tsx so the build-time prerender
// (scripts/prerender-home.mjs -> src/prerender-entry.tsx) can render the same
// App with react-dom/server without pulling in the client mount logic.
//
// Render-phase purity contract (see src/prerenderContext.ts): while the
// PrerenderedHomeProvider is active — during prerender and during the client's
// first hydration render — this tree must not read window.location-derived
// state the server cannot know (localStorage, matchMedia, Date.now). The
// prerender entry mocks a bare `window.location` at "/" before importing this
// module, so pathname reads stay valid in both environments.
...
import { trackEvent, providerFromRepoUrl } from "./analytics";
...
// The homepage extension promo comes from the injection context (see
// prerenderContext.ts): lazy on plain SPA loads, eager on the prerendered
// home so the served markup and the hydration render are identical.
...
function useNearViewport<T extends HTMLElement>(rootMargin = "600px") {
  ...
  // Prerendered home: the deferred sections' HTML already shipped inside the
  // page, so the first render (server and hydration) must include them; the
  // hydration contract forbids the client's first render from being emptier
  // than the served markup. Eager is also the steady state there — unmounting
  // already-painted content after hydration would collapse the page.
  const prerendered = usePrerenderedHome();
  const [isNear, setIsNear] = useState(prerendered);
...
// Node (build-time prerender) has no document/window: there is no SSR summary
// and no route, which is exactly the homepage case — the bundled demo report
// is the prerender's seed, matching the client hydration render on "/".
const ssrSeed = typeof document === "undefined" ? null : seedReportFromSsrSummary();
...
const seedReport = ssrSeed
  ...
  : typeof window === "undefined" || !window.location.pathname.startsWith("/github/")
    ? normalizeReport(initialReportData as unknown as Report)
    : null;

// Reads the pre-hydration DOM value of a form field so a visitor who typed
// into the prerendered page before React arrives keeps their text. This CANNOT
// happen in the useState initializer: the first client render must match the
// served tree byte for byte (the server rendered with the URL-derived value,
// and structure below the form — the demo note, the auto-run — depends on it),
// so adoption runs in a layout effect instead. React's hydration-time
// restoreControlledState rewrites the input to the server value first; the
// layout effect then re-writes the typed value back, both synchronously before
// the browser paints, so the visitor never sees the round trip.
function adoptPreHydrationInput(id: string, current: string): string {
  if (typeof document === "undefined") return current;
  const element = document.getElementById(id);
  return element instanceof HTMLInputElement && element.value !== current ? element.value : current;
}

export function App() {
  const { t, i18n } = useTranslation();
  const ExtensionPromo = useExtensionPromo();
  ...
  // Pre-hydration typing adoption (see adoptPreHydrationInput). This also
  // means an adopted (typed) value does NOT trigger the auto-run below: the
  // [] effect closes over the first render's URL-derived repoUrl, and typing
  // is not a submit.
  useLayoutEffect(() => {
    const typedUrl = adoptPreHydrationInput("repo-url", repoUrl);
    const typedRef = adoptPreHydrationInput("repo-ref", refName);
    if (typedUrl !== repoUrl) setRepoUrl(typedUrl);
  ...
  // Recent chips start empty and load from localStorage after mount: the
  // stored list is visitor-private state the prerendered HTML cannot contain,
  // and reading it during the first render would make the hydration tree
  // diverge from the served markup. The chips appear once hydration settles.
  const [recentRepos, setRecentRepos] = useState<RecentEntry[]>([]);
  useEffect(() => {
    ...
    <>
      ...
      <main id="main" className="page">
        ...
        {!isReportRoute ? <section id="extension" className="extension-promo">
          ...
          <Suspense fallback={null}><ExtensionPromo compact /></Suspense>
        </section> : null}
        ...
        {/* Real page content, not crawler-only markup: the homepage FAQ renders
            from the same shared module (src/homeFaq.ts) as the FAQPage JSON-LD
            below, so the visible answers and the structured data cannot drift.
            Replaces the request-time FAQ injection the Pages Function used to
            perform — the prerendered page ships it as first-class HTML. */}
        <HomeFaqSection />
        </>}
...
function HomeFaqSection() {
  ...
  return (
    <section className="section-compact home-faq" id="faq">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: faqJsonLdScript(i18n.language) }} />
      <div className="section-h">
        <h2>{t("homeFaq.title")}</h2>
        <span className="sub">{t("homeFaq.subtitle")}</span>
      </div>
